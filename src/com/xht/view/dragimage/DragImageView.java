package com.xht.view.dragimage;

import java.util.List;
import com.xht.util.LocalManager;
import com.xht.util.PointUtil;
import com.xht.verifinger.R;
import com.xht.view.popmenu.ActionItem;
import com.xht.view.popmenu.TitlePopup.OnItemOnClickListener;
import com.xht.view.popmenu.TitlePopup;
 ...
 * 
 */
public class DragImageView extends ImageView implements OnItemOnClickListener {
	...
	private TitlePopup titlePopup; // 弹窗

	private float image_x; // 当前点击位置对应的图像上的像素坐标x

	private float image_y; // 当前点击位置对应的图像上的像素坐标y

	private float pop_x; // 弹出框的坐标x
	private float pop_y; // 弹出框的坐标y

	public List<String> Pointlist; // 存特征点
	...
	private boolean mIsShowDelete = false; // 判断已显示弹窗

	public Bitmap moribmp; //原始指纹
	public Bitmap mfeabmp; //增强指纹
	public Bitmap mcurrentbmp;
	public int mImageType; //显示方式
	...
	//
	public void initView(Bitmap ori_bmp,Bitmap fea_bmp,int mType) {
		/** 获取图片宽高 **/
		bitmap_W = ori_bmp.getWidth();
		bitmap_H = ori_bmp.getHeight();
		...
		titlePopup.addAction(new ActionItem(getContext(), "删除特征",
				R.drawable.delete));
		moribmp = ori_bmp;
		mfeabmp = fea_bmp;
		mImageType = mType;
		String pointStr = LocalManager.getLocalStr(PointUtil.CURRENT_POINT);
		Pointlist = PointUtil.getPointArray(pointStr);
		updateShowImage(mImageType);
	}
	
	private void updateCurrentBitmap(int mtype)
	{
		if(mtype==1||mtype==2)
			mcurrentbmp = moribmp;
		else
			mcurrentbmp = mfeabmp;
		bitmap_H = mcurrentbmp.getHeight();
		bitmap_W = mcurrentbmp.getWidth();	
	}
	...
	// 坐标转换
	private void getXYinImage(MotionEvent event) {
		float image_w = mcurrentbmp.getWidth();
		float image_h = mcurrentbmp.getHeight();
		float r, l, b, t;
		r = this.getRight();
		l = this.getLeft();
		b = this.getBottom();
		...
		float view_w = r - l;
		float view_h = b - t;
		float scale = view_w / image_w;
		float simage_h = image_h * scale;
		float x = event.getX();
		float y = event.getY();
		image_x = x / scale;
		image_y = (simage_h / 2 + (y - view_h / 2)) / scale;
	}

	private void getXYinScreen(float image_x, float image_y) {
		float image_w = mcurrentbmp.getWidth();
		float image_h = mcurrentbmp.getHeight();
		float r, l, b, t;
		r = this.getRight();
		...
		float scale = view_w / image_w;
		float simage_h = image_h * scale;
		pop_x = image_x * scale + l;
		pop_y = view_h/2- (simage_h/2 - image_y*scale)+t;
	}

	public void updateShowImage(int mtype) {
		mImageType = mtype;
		switch (mtype) {
		case 1:
			this.setImageBitmap(moribmp);
			break;
		case 2:
			Bitmap PointBitmap = PointUtil.AddPointToBitmap(Pointlist, moribmp);
			this.setImageBitmap(PointBitmap);
			break;
		case 3:
			this.setImageBitmap(mfeabmp);
			break;
		case 4:
			PointBitmap = PointUtil.AddPointToBitmap(Pointlist, mfeabmp);
			this.setImageBitmap(PointBitmap);
			break;
		default:
			break;
		}
		updateCurrentBitmap(mImageType);
	}

	private boolean isFeaturePoint() {
		double x = 0, y = 0;
		double minR = 10000;
		for (int i = 0; i < Pointlist.size(); i++) {
			...
			if (Math.abs(x - image_x) <= PointUtil.POINT_WIDTH
					&& Math.abs(y - image_y) <= PointUtil.POINT_WIDTH) {
				double curR = (x - image_x) * (x - image_x) + (y - image_y)
						* (y - image_y);
				if (curR < minR) {
					minR = curR;
					mdeleteIndex = i / 3;
	...
	void onTouchDown(MotionEvent event) {
		...
		getXYinImage(event);
		if (isFeaturePoint() && !mIsShowDelete&&(mImageType==2||mImageType==4)) {
			titlePopup.setItemOnClickListener(this);
			float point_x = Float.parseFloat(Pointlist.get(mdeleteIndex*3));
			float point_y = Float.parseFloat(Pointlist.get(mdeleteIndex*3+1));
			getXYinScreen(point_x, point_y);
			titlePopup.show(this, pop_x-7, pop_y+30);
		}
	...
	public void onItemClick(ActionItem item, int position) {
		...
		PointUtil.DeletePointFromArray(Pointlist, mdeleteIndex);
		updateShowImage(mImageType);
	}
