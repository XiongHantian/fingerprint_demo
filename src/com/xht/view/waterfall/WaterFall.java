...
import com.xht.verifinger.MatchResults;
...
public class WaterFall extends ScrollView {
	...
	private void init() {
		...
		colCount = 2;// 默认情况下是4列
		pageCount = MatchResults.RESULTNUM;// 默认每次加载30个瀑布流单元
		capacity = MatchResults.RESULTNUM;// 默认容纳10000张图
		random = new Random();
		colWidth = getResources().getDisplayMetrics().widthPixels / colCount;
		rowHeight = getResources().getDisplayMetrics().heightPixels / 3;
	 ...
	 * 在外部调用 第一次装载页面 必须调用
	 */
	public void setup(List<String> list1, List<String> list2) {
		containerLayout = new LinearLayout(getContext());
		containerLayout.setBackgroundColor(0X00000000);
		...
		for (int i = 0; i < colCount; i++) {
			...
			int padwidth = 5;
			colLayout.setPadding(padwidth, padwidth, padwidth, padwidth);
			colLayout.setOrientation(LinearLayout.VERTICAL);
			colLayout.setBackgroundColor(0X00000000);
		...
		if (t > oldt) {// 向下滚动
			if (t > 2 * viewHeight) {
				for (int i = 0; i < colCount; i++) {
					...
					// 回收上方超过两屏bitmap
					FlowingView topItem = (FlowingView) colLayout
							.getChildAt(currentTopLineIndex[i]);
					if (topItem.getFootHeight() < t - 2 * viewHeight) {
						topItem.recycle();
						currentTopLineIndex[i]++;
					}
					// 重载下方进入(+1)两屏以内bitmap
					FlowingView bomItem = (FlowingView) colLayout
							.getChildAt(Math.min(currentBomLineIndex[i] + 1,
									bomLineIndex[i]));
					if (bomItem.getFootHeight() <= t + 3 * viewHeight) {
						bomItem.reload();
						currentBomLineIndex[i] = Math.min(
								currentBomLineIndex[i] + 1, bomLineIndex[i]);
					}
				}
			}
		} else {// 向上滚动
			for (int i = 0; i < colCount; i++) {
				...
				// 回收下方超过两屏bitmap
				FlowingView bomItem = (FlowingView) colLayout
						.getChildAt(currentBomLineIndex[i]);
				if (bomItem.getFootHeight() > t + 3 * viewHeight) {
					bomItem.recycle();
					currentBomLineIndex[i]--;
				}
				// 重载上方进入(-1)两屏以内bitmap
				FlowingView topItem = (FlowingView) colLayout.getChildAt(Math
						.max(currentTopLineIndex[i] - 1, 0));
				if (topItem.getFootHeight() >= t - 2 * viewHeight) {
					topItem.reload();
					currentTopLineIndex[i] = Math.max(
							currentTopLineIndex[i] - 1, 0);
				}
			}
		}
	...
	private static class DelayHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			// 判断当前滑动到的位置，进行不同的处理
			if (waterFall.getScrollY() + waterFall.getHeight() >= waterFall
					.getMaxColHeight() - 20) {
				// 滑动到底部，添加下一页内容
	...
	private void addNextPageContent(boolean isFirstPage) {
		...
		for (int i = pageCount * currentPage; i < pageCount * (currentPage + 1)
				&& i < capacity; i++) {
	...
	private class PrepareFlowingViewRunnable implements Runnable {
		...
		@Override
		public void run() {
			FlowingView flowingView = new FlowingView(getContext(), id+1,
					colWidth,rowHeight);
			String imageFilePath = LocalManager.LOCAL_AVATAR_CACHE + imgidlist.get(id) + ".jpg";
			flowingView.setImageFilePath(imageFilePath);
			flowingView.setScoreAndUID(scorelist.get(id), imgidlist.get(id));
			flowingView.loadImage();
			addItemHandler.sendMessage(addItemHandler.obtainMessage(0x00,
					flowingView));
		}
